...
Original file is located at
    https://colab.research.google.com/drive/1Slp9k_21oMmg4rvxjfp0w7aNQv91QrtN
...
# LSTM을 이용한 Granger Causality 계산
class cLSTM(nn.Module):
    ...
    # Granger causality 찾아내기
    # Entry(i, j) -> j가 i의 원인인지 여부
    def GC(self, threshold=True):
        ...
        Returns:
          GC: (p x p) matrix. Entry (i, j) indicates whether variable j is  # j가 i의 원인인지 여부
...
# 라쏘
def lasso_regularize(network, lam):
  '''Apply lasso penalty at linear layer and hidden-hidden weigths.'''
  return  lam * (
        torch.sum(abs(network.linear.weight )) +
        torch.sum(abs(network.lstm.weight_hh_l0)))

# 엘라스틱넷
def elastic_regularize(network, lam):
  '''Apply lasso penalty at linear layer and hidden-hidden weigths.'''
  r = 0.5
  return  lam * (
        torch.sum(abs(network.linear.weight )) +
        r * torch.sum(abs(network.lstm.weight_hh_l0)) +
        ((1 - r) / 2) * torch.sum(network.lstm.weight_hh_l0 ** 2))
...
def train_model_ista_lasso(clstm, X, context, lr, max_iter, lam=0, lam_ridge=0,
                     lookback=5, check_every=50, verbose=1):
    '''
    Train model with Adam.
    Args:
    ...
    pred = [clstm.networks[i](X)[0] for i in range(p)]
    loss = sum([loss_fn(pred[i][:, :, 0], Y[:, :, i]) for i in range(p)])
    ridge = sum([lasso_regularize(net, lam_ridge) for net in clstm.networks])
    smooth = loss + ridge
    ...
    for it in range(max_iter):
        ...
        ridge = sum([lasso_regularize(net, lam_ridge)
             for net in clstm.networks])
...
def train_model_ista_ridge(clstm, X, context, lr, max_iter, lam=0, lam_ridge=0,
                     lookback=5, check_every=50, verbose=1):
    '''
    Train model with Adam.
    Args:
...
def train_model_ista_elastic(clstm, X, context, lr, max_iter, lam=0, lam_ridge=0,
                     lookback=5, check_every=50, verbose=1):
    '''
    Train model with Adam.
    Args:
    ...
    ridge = sum([elastic_regularize(net, lam_ridge) for net in clstm.networks])
    smooth = loss + ridge
    ...
    for it in range(max_iter):
        ...
        ridge = sum([elastic_regularize(net, lam_ridge)
             for net in clstm.networks])
        smooth = loss + ridge
...
df_rate1 = pd.read_csv('granger_data_no1.csv', encoding="cp949")
df_rate2 = pd.read_csv('granger_data_no2.csv', encoding="cp949")

df_rate1 = df_rate1.drop(['date'], axis=1)
df_rate2 = df_rate2.drop(['date'], axis=1)
...
for col in diff1_cols:
  df_rate1[col] = difference(df_rate1[col])
  df_rate1[col] = difference(df_rate2[col])
...
cols = df_rate1.columns
cols

"""# 이재명 후보"""

rate_np = df_rate1.to_numpy()
rate_np.shape

rate = torch.tensor(rate_np[np.newaxis], dtype=torch.int32, device=device)
rate.shape

# Plot data
fig, axarr = plt.subplots(1, 2, figsize=(16, 5))
axarr[0].plot(rate_np[0])
axarr[0].set_xlabel('T')
axarr[0].set_title('Rate')
axarr[1].plot(rate_np[1:])
...
# Set up model
clstm_lasso = cLSTM(rate.shape[-1], hidden=100).cuda(device=device)

# Train with ISTA - lasso
train_loss_list = train_model_ista_lasso(
    clstm_lasso, rate, context=10, lam=10.0, lam_ridge=1e-2, lr=1e-3, max_iter=10000, check_every=50)

# Check learned Granger causality
GC_est_lasso = clstm_lasso.GC().cpu().data.numpy()

# print('True variable usage = %.2f%%' % (100 * np.mean(GC)))
print('Estimated variable usage = %.2f%%' % (100 * np.mean(GC_est_lasso)))
...
axarr[1].imshow(GC_est_lasso, cmap='Blues', vmin=0, vmax=1, extent=(0, len(GC_est_lasso), len(GC_est_lasso), 0))
...
print(GC_est_lasso[0])

# Set up model
clstm_ridge = cLSTM(rate.shape[-1], hidden=100).cuda(device=device)

# Train with ISTA - ridge
train_loss_list = train_model_ista_ridge(
    clstm_ridge, rate, context=10, lam=10.0, lam_ridge=1e-2, lr=1e-3, max_iter=10000, check_every=50)

# Check learned Granger causality
GC_est_ridge = clstm_ridge.GC().cpu().data.numpy()

# print('True variable usage = %.2f%%' % (100 * np.mean(GC)))
print('Estimated variable usage = %.2f%%' % (100 * np.mean(GC_est_ridge)))
# print('Accuracy = %.2f%%' % (100 * np.mean(GC == GC_est)))
...
axarr[1].imshow(GC_est_ridge, cmap='Blues', vmin=0, vmax=1, extent=(0, len(GC_est_ridge), len(GC_est_ridge), 0))
axarr[0].set_title('GC estimated')
axarr[1].set_ylabel('Affected series')
axarr[1].set_xlabel('Causal series')
axarr[1].set_xticks([])
...
print(GC_est_ridge[0])

# Set up model
clstm_elastic = cLSTM(rate.shape[-1], hidden=100).cuda(device=device)

# Train with ISTA - elastic
train_loss_list = train_model_ista_ridge(
    clstm_elastic, rate, context=10, lam=10.0, lam_ridge=1e-2, lr=1e-3, max_iter=10000, check_every=50)

# Check learned Granger causality
GC_est_elastic = clstm_elastic.GC().cpu().data.numpy()

# print('True variable usage = %.2f%%' % (100 * np.mean(GC)))
print('Estimated variable usage = %.2f%%' % (100 * np.mean(GC_est_elastic)))
# print('Accuracy = %.2f%%' % (100 * np.mean(GC == GC_est)))
...
axarr[1].imshow(GC_est_elastic, cmap='Blues', vmin=0, vmax=1, extent=(0, len(GC_est_elastic), len(GC_est_elastic), 0))
axarr[0].set_title('GC estimated')
axarr[1].set_ylabel('Affected series')
axarr[1].set_xlabel('Causal series')
axarr[1].set_xticks([])
...
print(GC_est_elastic[0])

print(GC_est_lasso[0])
print(GC_est_ridge[0])
print(GC_est_elastic[0])

GC_est_lasso[0] == GC_est_ridge[0]

GC_est_ridge[0] == GC_est_elastic[0]

GC_est_lasso[0] == GC_est_elastic[0]

"""# 윤석열 후보"""

rate_np = df_rate2.to_numpy()
rate_np.shape
